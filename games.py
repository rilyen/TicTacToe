...
def minmax_cutoff(game, state):
    ...
    def max_value(state, d):
        ...
        if game.terminal_test(state) or d >= game.end:
            return game.utility(state, player)
        v = -np.inf
        for a in game.actions(state):
            v = max(v, min_value(game.result(state, a), time.perf_counter()))
        return v
    ...
    def min_value(state, d):
        ...
        v = np.inf
        for a in game.actions(state):
            v = min(v, max_value(game.result(state, a), time.perf_counter()))
        return v
...
def alpha_beta_cutoff(game, state):
    ...
    def max_value(state, alpha, beta, depth):
        if game.terminal_test(state) or depth >= game.d:
            return game.utility(state, player)
        # print("Your code goes here -3pt")
        v = -np.inf
        for a in game.actions(state):
    ...
    # print("Your code goes here -10pt")
    for a in game.actions(state):
        v = min_value(game.result(state, a), alpha, beta, time.perf_counter())
        if v > alpha:
            alpha = v
            best_action = a
...
class TicTacToe(Game):
    ...
    def display(self, state):
        ...
        for x in range(0, self.size):
            for y in range(1, self.size + 1):
                print(board.x((self.size - x, y), '.'), end=' ')
            print()
    ...
    def eval1(self, state):
        ...
        utility = 0
        for move in state.board:
            if state.board[move] == 'X':
                utility += possiblekComplete(move, state.board, 'X', self.k)
            elif state.board[move] == 'O':
                utility -= possiblekComplete(move, state.board, 'O', self.k)
